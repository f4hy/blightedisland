...
import game_history
from config import (
    APP_TITLE, APP_SUBTITLE, CHART_COLORS, CHART_HEIGHT,
    WIN_LABEL, LOSS_LABEL, DESYNC_LABEL, RESULT_OPTIONS
)
...
def render_stats_tab(games: List[game_history.Game]) -> None:
    ...
    win_rate = round(num_wins / (num_wins + num_losses) * 100, 1) if (num_wins + num_losses) > 0 else 0
...
def render_random_spirit_tab() -> None:
    ...
    selected_complexity = st.radio("Filter by complexity:", complexities, horizontal=True)
        ...
        with col2:
            ...
            if games:
                ...
                if spirit_key in spirit_stats:
                    ...
                    st.write(f"**Win Rate:** {stats['win_pct']}% ({stats['wins']}/{stats['total']} games)")
                else:
...
def render_random_adversary_tab(games: List[game_history.Game]) -> None:
    ...
    with col2:
        if st.button("⚖️ Weighted Random Adversary", 
    ...
    if picked:
        ...
        if adv_key in adversary_stats:
            ...
            st.metric("Win Rate", f"{stats['win_pct']}% ({stats['wins']}/{stats['total']} games)")
...
def render_record_game_tab() -> None:
    ...
    # Notes field
    notes = st.text_area("Game Notes (optional)", 
                        placeholder="Enter any observations about the game...")
    ...
    # Validation
    all_inputs_valid = (
        date_played is not None and
        selected_adversary is not None and
        len(player_selections) == selected_count_num and
        result is not None
    )
...
def render_game_history_tab(games: List[game_history.Game]) -> None:
    ...
    if not games:
        st.info("No games recorded yet! Use the 'Record Game' tab to add your first game.")
        return
    ...
    if search_term:
        ...
        filtered_games = [
            g for g in games if (
                search_lower in g.adversary.name.lower() or
                any(search_lower in ps.player.name.lower() for ps in g.players_played) or
                any(search_lower in str(ps.spirit).lower() for ps in g.players_played)
            )
        ]
    
    # Sort the games
    sorted_games = sorted(filtered_games, key=sort_func, reverse=(sort_by == "Newest first"))
    ...
    for i, g in enumerate(sorted_games):
        ...
        with st.expander(
            f"{g.date_played} | {g.adversary} | {player_str}",
            expanded=False
        ):
            g.show()
            ...
            with col1:
                if st.button("🗑️ Delete", key=f"delete_game_{i}", help="Delete this game record"):
...
def render_settings_tab(games: List[game_history.Game]) -> None:
    ...
    with import_col:
        ...
        if uploaded_file is not None:
            ...
            if st.button("📤 Import Games", type="primary"):
                ...
                if failed > 0:
                    st.warning(f"Failed to import {failed} games due to validation errors.")
    ...
    # Display app info
    st.write("#### ℹ️ About")
    st.write("**Blighted Island** is a stat tracking app for the Spirit Island board game.")
    st.write("Version: 1.1.0")
    st.write("Made with Streamlit and Python")
    
    # Reset application data
    st.write("#### 🔄 Reset Application")
    if st.button("🗑️ Reset All Data", type="primary", help="This will delete all your game history!", disabled=True):
        # This would clear all data in a real implementation
        st.warning("This would delete all your data. Reset functionality not implemented in this demo.")
...
def display_win_loss_trend(games: List[game_history.Game]) -> None:
    ...
    date_results = [
        {"date": g.date_played, "result": "Win" if g.won else "Loss" if g.won is False else "Desync"}
    ...
    if not df.empty:
        ...
        win_rate_chart = alt.Chart(df).mark_line(point=True).encode(
