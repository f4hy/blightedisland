...
class Game(BaseModel, frozen=True):
    ...
    def show(self) -> None:
        ...
        st.write("WON! 🎉" if self.won else "Lost 😢" if self.won is False else "Desync ⚠️")
        
        # Show date and adversary info
        col1, col2 = st.columns(2)
        with col1:
            st.write(f"**Date:** {self.date_played}")
            st.write(f"**Adversary:** {self.adversary.display_name} (Level {self.adversary.level})")
...
class GameFilters(BaseModel):
    ...
    def filter_games(self, games: List[Game]) -> List[Game]:
        ...
        if self.player:
            filtered = [
                g for g in filtered if self.player in {p.player for p in g.players_played}
            ]
